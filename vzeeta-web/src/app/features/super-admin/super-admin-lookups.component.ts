...
import { LookupItemDialogComponent } from './lookup-item-dialog.component';
  ...
  template: `
    <div class="app-page lookup-page">
      ...
      <mat-tab-group class="lookup-tabs" animationDuration="180ms">
        ...
        <mat-tab [label]="'LOOKUPS.TAB_REFERENCE' | translate">
          <mat-accordion class="lookup-panel-stack" multi>
            <mat-expansion-panel class="lookup-box" *ngFor="let list of lists">
              <mat-expansion-panel-header>
                <mat-panel-title>
                  <span class="material-icons">{{ list.icon }}</span>
                  <span>{{ list.labelKey | translate }}</span>
                </mat-panel-title>
              </mat-expansion-panel-header>

              <div class="lookup-box-head">
                <div>
                  <h2>{{ list.labelKey | translate }}</h2>
                  <p>{{ 'LOOKUPS.ITEM_COUNT' | translate:{ count: items[list.type].length } }}</p>
                </div>
                <div class="lookup-head-actions">
                  <button mat-flat-button color="primary" type="button" (click)="openAdd(list)">
                    <mat-icon>add</mat-icon>
              ...
              <div class="loading-center" *ngIf="loading[list.type]">
                <mat-spinner diameter="32"></mat-spinner>
              </div>

              <div class="app-table-wrap" *ngIf="!loading[list.type] && items[list.type].length > 0">
                <table class="app-data-table">
                  <thead>
                    <tr>
                      <th class="table-index-col">#</th>
                  ...
                  <tbody>
                    <tr *ngFor="let item of pagedItems(list.type); let i = index">
                      <td class="table-index-col">{{ i | tableRowIndex:pageIndex[list.type]:pageSize }}</td>
                      <td>{{ nameOf(item) }}</td>
                      <td><code class="code-tag">{{ item.code }}</code></td>
                      <td>
                        <span class="status-badge" [attr.data-status]="item.active ? 'ACTIVE' : 'INACTIVE'">
                          {{ (item.active ? 'LOOKUPS.ACTIVE' : 'LOOKUPS.INACTIVE') | translate }}
                        </span>
                      </td>
                      <td class="actions-col">
                        <div class="app-row-actions">
                          <button type="button" class="app-icon-btn info" (click)="openEdit(list, item)" [matTooltip]="'ACTIONS.EDIT' | translate">
                            <mat-icon>edit</mat-icon>
                          </button>
                          <button
                            type="button"
                            class="app-icon-btn danger"
                            [disabled]="item.locked || deletingId === item.id"
                            (click)="remove(list, item)"
                            [matTooltip]="'ACTIONS.DELETE' | translate">
                            <mat-icon>delete</mat-icon>
                          </button>
                          <mat-icon class="lock-icon" *ngIf="item.locked" [matTooltip]="'LOOKUPS.LOCKED_ROW_HINT' | translate">lock</mat-icon>
                        </div>
                      </td>
                    </tr>
                  </tbody>
              ...
              <p class="app-empty-state-inline" *ngIf="!loading[list.type] && !items[list.type].length">
                {{ 'COMMON.NO_DATA' | translate }}
              </p>

              <app-table-pager
                *ngIf="items[list.type].length > 0"
                [length]="items[list.type].length"
                [pageSize]="pageSize"
                [pageIndex]="pageIndex[list.type]"
                (pageIndexChange)="pageIndex[list.type] = $event">
              </app-table-pager>
            </mat-expansion-panel>
          </mat-accordion>
        </mat-tab>
